...
function TramitesMatricula() {
  ...
  return (
    ...
    <div className="absolute inset-0 flex mt-[50px] flex-col justify-center">
        ...
        {enlaces.map((e)=>(<Link className='bg-[#0F0D3C] m-[15px] text-center text-[#f2f2f2] rounded-3xl text-[18px] flex justify-center items-center h-[50px] xl:text-2xl w-[320px] xl:w-[500px] transform scale-100 transition duration-500 hover:scale-105'
            key={e.id}
            to={e.href}
            >{e.link}</Link> ))}
        </div>
