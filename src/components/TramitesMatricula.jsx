...
function TramitesMatricula() {
  ...
  return (
    ...
    <motion.div initial={{ opacity: 0, x: -100 }} 
        ...
        {enlaces.map((e)=>(<Link className='bg-[#0F0D3C] m-[15px] text-center text-[#f2f2f2] rounded-3xl text-[18px] flex justify-center items-center h-[50px] xl:text-2xl w-[320px] xl:w-[500px] transform scale-100 transition duration-500 hover:scale-110'
            key={e.id}
            to={e.href}
            >{e.link}</Link> ))}
        </div>
