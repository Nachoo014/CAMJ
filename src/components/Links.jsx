  ...
  return (
    <div className='p-[50px] h-[100%] w-full bg-[#ffe7d1] grid grid-cols-1 xl:grid-cols-5 gap-3'>
        {enlaces.map((e) => (
              ...
              to={e.href}
            >
              <motion.p initial={{ opacity: 0, y: -100 }} whileInView={{ opacity: 1, y:0, transition: {duration: 0.4} }} className='text-2xl md:text-[75px] '>{e.icono}</motion.p>
              <motion.span initial={{ opacity: 0, y: -100 }} whileInView={{ opacity: 1, y:0, transition: {duration: 0.4} }} >{e.link}</motion.span >
            </Link>
            ))}
    </div>
  )
